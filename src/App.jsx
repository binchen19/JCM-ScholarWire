    ...
    <a href={article.doi ? `https://doi.org/${article.doi}` : (article.url || "#")}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <h3 style={{ fontSize: 15, fontWeight: 600, lineHeight: 1.45, margin: "0 0 6px", fontFamily: "'Source Serif 4', Georgia, serif", color: "#1a1a1a" }}>
    ...
    <div style={{ borderBottom: "1px solid #eee", padding: "16px 0", opacity: isPastEvent ? 0.5 : 1 }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        <div style={{
          ...
          background: isPastEvent ? "#ddd" : "#457B9D",
        ...
        <div style={{ flex: 1 }}>
          ...
          {conf.cfpDeadline && (
            <p style={{ fontSize: 11, color: isPastCfp ? "#999" : "#E63946", margin: "0 0 2px", fontFamily: "'JetBrains Mono', monospace", textDecoration: isPastCfp ? "line-through" : "none" }}>
        ...
        {onDelete && (
          <button onClick={() => onDelete(conf.id)}
            style={{ background: "none", border: "none", cursor: "pointer", color: "#ccc", fontSize: 16, padding: "4px 8px", alignSelf: "flex-start" }}
...
export default function JournalTracker() {
  const [journals, setJournals] = useState(() => {
    try { const s = localStorage.getItem("jcm-journals"); return s ? JSON.parse(s) : DEFAULT_JOURNALS; }
    catch { return DEFAULT_JOURNALS; }
  });
  const [articles, setArticles] = useState([]);
  const [conferences, setConferences] = useState(() => {
    try { const s = localStorage.getItem("jcm-conf-v6"); return s ? JSON.parse(s) : DEFAULT_CONFERENCES; }
    catch { return DEFAULT_CONFERENCES; }
  });
  const [cfps, setCfps] = useState(() => {
    try { const s = localStorage.getItem("jcm-cfps-v6"); return s ? JSON.parse(s) : []; }
    catch { return []; }
  });
  ...
  useEffect(() => { try { localStorage.setItem("jcm-conf-v6", JSON.stringify(conferences)); } catch {} }, [conferences]);
  useEffect(() => { try { localStorage.setItem("jcm-cfps-v6", JSON.stringify(cfps)); } catch {} }, [cfps]);
  ...
  useEffect(() => {
    if (!hasFetched.current) { hasFetched.current = true; fetchArticles(); }
  }, [fetchArticles]);
  ...
  const filteredArticles = articles.filter(a => {
    ...
    if (searchTerm) {
      ...
      return (a.title || "").toLowerCase().includes(s) || (a.authors || []).some(au => au.toLowerCase().includes(s));
    }
    return true;
  });
  ...
  const addCfp = () => {
    ...
    setNewCfp({ title: "", type: "Special Issue", venue: "", deadline: "", url: "" });
    setShowAddCfp(false);
  };

  const addConf = () => {
    if (!newConf.name || !newConf.fullName) return;
    setConferences([...conferences, { ...newConf, id: Date.now() }]);
    setNewConf({ name: "", fullName: "", location: "", dates: "", startDate: "", cfpDeadline: "", cfpNote: "", url: "" });
    ...
    <div style={{ fontFamily: "'Source Serif 4', Georgia, serif", maxWidth: 780, margin: "0 auto", padding: "0 20px", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #eee", padding: "12px 0" }}>
        {[
          ...
          { label: "Upcoming", value: upcomingConfCount },
        ].map((s, i) => (
      ...
      <nav style={{ display: "flex", gap: 0, borderBottom: "1px solid #eee" }}>
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            fontWeight: tab === t ? 700 : 500, color: tab === t ? "#1a1a1a" : "#999",
      ...
      {tab === "Feed" && (
        <div style={{ animation: "fadeIn 0.3s ease" }}>
          ...
          {!loading && !fetchError && filteredArticles.length === 0 && articles.length > 0 && (
            <p style={{ textAlign: "center", color: "#999", padding: "30px 0", fontSize: 13, fontFamily: "'JetBrains Mono', monospace" }}>No articles match your filters.</p>
          )}
        </div>
      )}
            ...
            <div style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 4, padding: 16, marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <div style={{ flex: "1 1 250px" }}>
                  ...
                  <input value={newJournal.name} onChange={e => setNewJournal({ ...newJournal, name: e.target.value })} placeholder="e.g. Journalism & Mass Communication Quarterly" style={inputStyle} />
                </div>
                <div style={{ flex: "0 1 180px" }}>
                  <label style={labelStyle}>ISSN (print,online)</label>
                  <input value={newJournal.issn} onChange={e => setNewJournal({ ...newJournal, issn: e.target.value })} placeholder="e.g. 1077-6990,2161-4326" style={inputStyle} />
                </div>
              ...
              <p style={{ fontSize: 11, color: "#aaa", marginTop: 8, fontFamily: "'JetBrains Mono', monospace" }}>
                Find ISSNs at <a href="https://portal.issn.org" target="_blank" rel="noopener noreferrer" style={{ color: "#E63946" }}>portal.issn.org</a>. Use both print & online ISSNs separated by comma for best results.
              </p>
            </div>
          )}
          ...
          {journals.map(j => (
            <div key={j.issn} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #eee" }}>
              ...
              <button onClick={() => setJournals(journals.filter(x => x.issn !== j.issn))}
                style={{ background: "none", border: "1px solid #eee", borderRadius: 3, padding: "4px 12px", cursor: "pointer", color: "#999", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}
      ...
      {tab === "Conferences" && (
        <div style={{ animation: "fadeIn 0.3s ease", padding: "16px 0" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ flex: "0 1 100px" }}>
                    ...
                    <input value={newConf.name} onChange={e => setNewConf({ ...newConf, name: e.target.value })} placeholder="e.g. WAPOR" style={inputStyle} />
                  </div>
                  <div style={{ flex: "1 1 300px" }}>
                    <label style={labelStyle}>Full Name</label>
                    <input value={newConf.fullName} onChange={e => setNewConf({ ...newConf, fullName: e.target.value })} placeholder="e.g. World Association for Public Opinion Research" style={inputStyle} />
                  </div>
                </div>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ flex: "1 1 200px" }}>
                    <label style={labelStyle}>Location</label>
                    <input value={newConf.location} onChange={e => setNewConf({ ...newConf, location: e.target.value })} placeholder="e.g. Berlin, Germany" style={inputStyle} />
                  </div>
                  <div style={{ flex: "0 1 160px" }}>
                    <label style={labelStyle}>Event Dates (text)</label>
                    <input value={newConf.dates} onChange={e => setNewConf({ ...newConf, dates: e.target.value })} placeholder="e.g. Jun 10–13, 2026" style={inputStyle} />
                  </div>
                  <div style={{ flex: "0 1 150px" }}>
                    <label style={labelStyle}>Start Date</label>
                    <input type="date" value={newConf.startDate} onChange={e => setNewConf({ ...newConf, startDate: e.target.value })} style={inputStyle} />
                  </div>
                </div>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ flex: "0 1 160px" }}>
                    <label style={labelStyle}>CFP Deadline</label>
                    <input type="date" value={newConf.cfpDeadline} onChange={e => setNewConf({ ...newConf, cfpDeadline: e.target.value })} style={inputStyle} />
                  </div>
                  <div style={{ flex: "1 1 250px" }}>
                    <label style={labelStyle}>CFP Note</label>
                    <input value={newConf.cfpNote} onChange={e => setNewConf({ ...newConf, cfpNote: e.target.value })} placeholder="e.g. Full papers, max 8000 words" style={inputStyle} />
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>URL (optional)</label>
                  <input value={newConf.url} onChange={e => setNewConf({ ...newConf, url: e.target.value })} placeholder="https://..." style={inputStyle} />
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={addConf} style={btnPrimary}>Add Conference</button>
          ...
          {filteredConfs.map(c => (
            <ConferenceCard key={c.id} conf={c} onDelete={(id) => setConferences(conferences.filter(x => x.id !== id))} />
          ))}
        </div>
      )}
            ...
            <div style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 4, padding: 16, marginBottom: 16 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div>
                  ...
                  <input value={newCfp.title} onChange={e => setNewCfp({ ...newCfp, title: e.target.value })} placeholder="e.g. Special Issue on Platform Governance" style={inputStyle} />
                </div>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: "1 1 200px" }}>
                    ...
                    <input value={newCfp.venue} onChange={e => setNewCfp({ ...newCfp, venue: e.target.value })} placeholder="e.g. New Media & Society" style={inputStyle} />
                  </div>
                  <div style={{ flex: "0 1 150px" }}>
                    <label style={labelStyle}>Submission Deadline</label>
                    <input type="date" value={newCfp.deadline} onChange={e => setNewCfp({ ...newCfp, deadline: e.target.value })} style={inputStyle} />
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>URL (optional)</label>
                  <input value={newCfp.url} onChange={e => setNewCfp({ ...newCfp, url: e.target.value })} placeholder="https://..." style={inputStyle} />
                </div>
                <div style={{ display: "flex", gap: 6 }}>
